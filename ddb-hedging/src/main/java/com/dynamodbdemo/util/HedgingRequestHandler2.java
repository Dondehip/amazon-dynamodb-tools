...
public class HedgingRequestHandler2 implements HedgingRequestHandler {
    ...
    public CompletableFuture<DDBResponse> hedgeRequests(
        ...
        CompletableFuture<DDBResponse> hedgedRequest = CompletableFuture.supplyAsync(() -> {
            ...
            logger.info(String.format("Scheduling delay #%.2f", schedulingDelay));


            if (cancelPending) {
                logger.debug("Check Before hedged request#{} can be initiated", requestNumber);
                ...
                if (completedFuture != null) {
                    logger.debug("Previous request already completed, skipping hedge request#{}", requestNumber);
                    return completedFuture.thenApply(response -> {
                        // Update the scheduling delay for the completed response
    ...
    private void cancelPendingRequests(List<CompletableFuture<DDBResponse>> allRequests, int requestNumber) {
        logger.debug("Request {} completed, cancelling other pending requests", requestNumber);
